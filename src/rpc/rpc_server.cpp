...
{
    std::function<void(minico::Socket *)> on_connection = std::bind(&RpcServer::on_connection, this, std::placeholders::_1);
...
void RpcServer::on_connection(minico::Socket *conn)
{
    ...
    while (true)
    {
        ...
        int rpc_request_message_len = connection->read(&rpc_header, sizeof(rpc_header));
